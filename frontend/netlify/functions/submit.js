  ...
  try {
    ...
    if (type === 'contact') {
      ...
      html = `
        ...
        <p style="color:#999;margin-top:16px;font-size:12px;">提交时间：${formatDate()}</p>
      `;
      ...
      return {
        statusCode: 400,
        body: JSON.stringify({ success: false, message: '未知的表单类型' }),
      };
    }
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ success: false, message: '提交失败，请稍后重试' }),
    };
  }
};
