...
import { Building2, User, Mail, Phone, Briefcase, ArrowLeft, Send, CheckCircle } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/submit', {
        ...
        body: JSON.stringify({ type: 'careers', data: formData }),
      });
        ...
        <FadeIn>
          <Card className="bg-[#1a1a2e] border-[#292929] max-w-md w-full text-center">
            <CardContent className="pt-8 pb-8">
              ...
              <h2 className="text-2xl font-bold text-white mb-2">申请提交成功</h2>
              <p className="text-[#9ca3af] mb-6">
                感谢您的关注！我们的人力资源团队将在3个工作日内与您联系。
              </p>
              <Link to="/">
        ...
        <FadeIn>
          <Card className="bg-[#1a1a2e] border-[#292929]">
            <CardHeader className="text-center pb-2">
              ...
              <CardTitle className="text-white text-2xl">加入我们</CardTitle>
              <CardDescription className="text-[#9ca3af]">
                加入智优康赛，与顶尖团队一起在AI搜索时代创造价值
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    placeholder="请描述您的专业技能、工作经验或其他想说明的内容"
                    value={formData.message}
                    onChange={(e) => setFormData({...formData, message: e.target.value})}
                    className="bg-[#0a0a0f] border-[#292929] text-white placeholder:text-[#6b7280] focus:border-[#6366f1] min-h-[100px]"
                  />
                </div>
