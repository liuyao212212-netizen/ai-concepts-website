...
const formatDate = () => new Date().toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' });
  ...
  try {
    ...
    if (type === 'contact') {
      ...
      html = `<h2>新的客户咨询</h2>
        ...
        </table><p style="color:#999;margin-top:16px;font-size:12px;">提交时间：${formatDate()}</p>`;
...
app.listen(PORT, '0.0.0.0', () => console.log(`[本地API服务] http://0.0.0.0:${PORT}`));
